...
from neuro_data_analysis.neural_data_lib import extract_all_evol_trajectory_dyna, pad_resp_traj, extract_all_evol_trajectory_dyna_multiwindow_nostats
...
window_configs = [(f"{step}ms_wdw", 
                   [range(i*step, (i+1)*step) for i in range(200//step)])
...
for window_length  in (10, 20, 25, 50):
    ...
    for commonmsk, commonmsk_title_str, commonmsk_str in [(validmsk & bothsucsmsk, "Valid & Both Success",  "valid_bothsucc"),
        ...
        figh = plot_normalized_response_trajectories(rsp_wdws, resp_col_multi_wdw, full_normalizer, 
                                                    [V1msk, V4msk, ITmsk], ["V1", "V4", "IT"], 
                                                    commonmsk, signif_alpha=0.05, signif_test=True, plot_individual_exp=False)
        figh.suptitle(
            f"Universal Max Normalized response {wdw_col_str} across blocks [{commonmsk_title_str} Sessions]")
        figh.tight_layout()
        saveallforms([outdir, figdir], f"univmaxnorm_resp_traj_{commonmsk_str}_area_sep_{wdw_col_str}_synopsis_annot_sigif_mcc", figh=figh)
        figh.show()
        ...
        figh.suptitle(
            f"Universal Max Normalized response {wdw_col_str} across blocks [{commonmsk_title_str} Sessions]")
        figh.tight_layout()
        saveallforms([outdir, figdir], f"univmaxnorm_resp_traj_{commonmsk_str}_area_sep_{wdw_col_str}_synopsis_annot_sigif_mcc_V4IT", figh=figh)
        figh.show()
#%%
...
for window_length  in (10, 20, 25, 50):
    ...
    for commonmsk, commonmsk_title_str, commonmsk_str in [(validmsk & bothsucsmsk, "Valid & Both Success",  "valid_bothsucc"),
        ...
        figh.suptitle(
            f"Universal Max Normalized response {wdw_col_str} across blocks [{commonmsk_title_str} Sessions]")
        figh.tight_layout()
        saveallforms([outdir, figdir], f"univmaxnorm_resp_traj_{commonmsk_str}_area_sep_{wdw_col_str}_synopsis_annot_sigif_mcc_fdr", figh=figh)
        figh.show()
...
for commonmsk, commonmsk_title_str, commonmsk_str in [(validmsk & bothsucsmsk, "Valid & Both Success",  "valid_bothsucc"),
    ...
    for colj, rsp_wdw in enumerate(rsp_wdws):
        ...
        for rowi, (msk_major, label_major) in enumerate(zip([V1msk, V4msk, ITmsk],
            ...
            for t in range(FC_arr.shape[1]):
                # two-sample t test
                _, p = stats.ttest_ind(FC_arr[:, t], BG_arr[:, t], equal_var=False)
                pvals.append(p)
            pvals = np.array(pvals)
            
            alpha = 0.05
            sig_mask = pvals < alpha
            # you can plot a small dot or line on the plot to show significance
            # for instance, put a black dot at y=1.3 (or some offset) whenever it's significant
            axs[rowi, colj].plot(np.where(sig_mask)[0],
                                np.ones(np.sum(sig_mask))*1.3,
                                'k.', markersize=4, label='p<0.05' if colj==0 and rowi==0 else "")
    ...
    plt.suptitle(
        f"Universal Max Normalized response {wdw_col_str} across blocks [{commonmsk_title_str} Sessions]")
    plt.tight_layout()
    saveallforms([outdir, figdir], f"univmaxnorm_resp_traj_{commonmsk_str}_area_sep_{wdw_col_str}_synopsis_annot_sigif", figh=figh)
    plt.show()
    ...
    plt.tight_layout()
    saveallforms([outdir, figdir], f"maxnorm_resp_traj_{commonmsk_str}_area_sep_{wdw_col_str}_synopsis_annot_sigif", figh=figh)
    plt.show()
