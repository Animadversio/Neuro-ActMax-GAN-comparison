...
del cnn
#%%
outdir = r"F:\insilico_exps\GAN_Evol_cmp\covtsr_similarity"
#%%
...
def layername_mapping(name):
    return name.replace(".blocks", "blocks").\
        replace(".layer","layer").\
        replace(".Bottleneck", "B")
#%%
unitdirs = list(Path(r"F:\insilico_exps\GAN_Evol_cmp").glob("tf_eff*"))\
    + list(Path(r"F:\insilico_exps\GAN_Evol_cmp").glob("resnet50_*"))
#%%
for unitdir in tqdm(unitdirs):
    unitname = Path(unitdir).name
    ...
    #%%
    try:
        target_netname = unitname.split("_.")[0]
        target_layername = unitname[len(target_netname)+1:].split("_")[0]
        targetRFkey = f"{target_netname}_{layername_mapping(target_layername)}"
        RFdict = fitRFdict[targetRFkey]
        RFmap = RFdict[0]
    except KeyError:
        RFmap = None
        print(f"KeyError: {unitname}")
    #%%
    cosine_data = {}
    ...
    for i, layer in enumerate(layernames1):
        ...
        cosine_data[netname1, layer] = cosine_map.numpy()
    ...
    for i, layer in enumerate(layernames2):
        ...
        cosine_data[netname2, layer] = cosine_map.numpy()
    ...
    if RFmap is not None:
        axs[0, 3].imshow(RFmap)
        axs[0, 3].set_title("RF map")
    plt.suptitle(f"{Path(unitdir).name}")
    plt.tight_layout()
    saveallforms(outdir, f"{Path(unitdir).name}_cosine_maps_FC{RND1:05d}_BG{RND2:05d}")
    plt.show()
    #%%
    pkl.dump(cosine_data, open(join(outdir, f"{Path(unitdir).name}_cosine_maps_FC{RND1:05d}_BG{RND2:05d}_data.pkl"), "wb"))
