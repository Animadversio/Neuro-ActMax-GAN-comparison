...
def sweep_dir(rootdir, unit_pattern, save_pattern):
    ...
    for unitdir in tqdm(unitdirs):
        ...
        for savefn in savefiles:
            ...
            initscores = scores_all[generations <= (generations.min() + 1)].mean()
            # print(scores_all[generations <= (generations.min() + 1)].shape)
...
rootdir = r"/n/holylfs06/LABS/kempner_fellow_binxuwang/Users/binxuwang/Projects/GAN_Evol_cmp"
...
df_evol = df_evol.astype({"GANname": str, "layer": str, "optimmethod": str, "netname": str,
                          "score": float, "maxscore": float, "initscore": float,
                          "maxstep": int, "RFresize": bool})
