...
saveroot = "/n/holylfs06/LABS/kempner_fellow_binxuwang/Users/binxuwang/Projects/BigGAN_Evol_cmp_Ecoset"
...
for rep in range(3):
    for target_module, layername in [
        ...
        for unit_i in range(100):
            T0 = time.time()
            Evol_results_INet = optimize_gan_codes(G_IN, CNN, fetcher, unit_idx=unit_i, init_code_std=0.01, init_sigma=0.06, print_freq=100)
            T1 = time.time()
            print(f"INet x ResNet50-linf8 {layername} ch{unit_i} rep{rep} : Act {Evol_results_INet['scores'][-25:].mean():.2f} time {T1-T0:.2f} sec")
            ...
            pkl.dump(Evol_results_INet, open(join(savedir, f"Evol_results_BigGAN_INet_resnet50_linf8_{layername}_ch{unit_i}_rep{rep}.pkl"), "wb"))
