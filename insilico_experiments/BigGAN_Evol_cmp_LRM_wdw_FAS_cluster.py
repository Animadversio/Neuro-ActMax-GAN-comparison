...
parser.add_argument('--Trng', type=int, default=(0, 3), nargs=2, help='Time range')
...
for iChannel in range(args.channel_rng[0], args.channel_rng[1]):
    savedir = join(saveroot, f"{args.model}-{layerkey}-Ch{iChannel:04d}_wdw")
    if args.RFresize:
        savedir += "_RFrsz"
    ...
    for GANname in ["fc6", "BigGAN"]:
        ...
        for trial in range(args.reps):
            ...
            print (f"{layerkey} Ch{iChannel} T{args.Trng[0]}-T{args.Trng[1]} | GAN {GANname} CMAES Trial | {trial}")
            methodlab = f"{GANname}_CMAES_wdwT{args.Trng[0]}-T{args.Trng[1]}" # alex-lrm3-{layerkey}_Ch{iChannel}_T{iT}_
            if GANname == "BigGAN":
            ...
            for i in range(args.steps,):
                ...
                scores = scores_dyn[:, args.Trng[0]:args.Trng[1] + 1].mean(axis=1)
